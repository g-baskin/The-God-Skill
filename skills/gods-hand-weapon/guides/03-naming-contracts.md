...
4. `skills/god/SKILL.md` -- search for the guardian name. There should be ZERO matches. A match here means the Angel is already registered with God's roster, which contradicts the `[ ]` state in the backlog. STOP and surface.
...
5. `skills/<weapon-name>/` -- check the folder does not already exist. If it does, STOP. Either the prior run crashed before reaching Phase 4, or there is a genuine name collision. Either way, surface for the caller.

6. `agents/<guardian-name>.md` -- check the file does not already exist. Same logic.
...
| Weapon folder (root) | `skills/<weapon-name>/` |
| Weapon `SKILL.md` | `skills/<weapon-name>/SKILL.md` |
| Weapon research folder | `skills/<weapon-name>/research/` |
| Weapon guides folder | `skills/<weapon-name>/guides/` |
| Angel file | `agents/<guardian-name>.md` |
| God's guide for this Angel | `skills/god/guides/<guardian-name>.md` |
...
`big-bang-earth`'s naming rules (`skills/big-bang-earth/`) document the upstream side of this contract. `gods-hand` is the downstream side; the rules must be enforced again here because nothing in the queue itself prevents a duplicate name from being authored if `big-bang-space` was used with stale uniqueness state.
