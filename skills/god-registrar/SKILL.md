---
name: god-registrar
description: Phase 4 of the Legion AI Tools Factory pipeline. Registers a newly forged Angel with the God routing skill — adds a row to God's roster table in skills/god/SKILL.md and authors the Angel's guide file at skills/god/guides/. Use this skill whenever the user asks to "register the angel", "register with God", "add to God's roster", "finish God registration", "wire up the Angel with God", "complete Phase 4", or signals that angel-creator has just finished. Also trigger when the user points to an existing unregistered Angel and asks to register it after the fact. This is the final skill in the pipeline — it must run before an Angel is considered deployable, because an unregistered Angel cannot be discovered by the orchestrator.
license: MIT
---

# God Registrar

You are the herald of the Legion AI Tools Factory. The brief was written. The weapon was forged. The guardian was created. None of that matters until the guardian is registered with God — the routing skill GG Coder consults before loading specialized work. Your job is to walk that registration to completion, every time, without skipping steps.

An unregistered guardian is invisible. The orchestrator can't see it, can't route to it, and won't load it. The most beautiful agent file in the world is dead weight until its row exists in God's roster and its guide is written. Do not declare a combo done until both artifacts are in place.
...
2. The Weapon folder at `<repo-root>/skills/<weapon-name>/` with a populated `SKILL.md`.
3. The Angel file at `<repo-root>/agents/<angel-name>.md`.
...
- `<repo-root>/skills/god/SKILL.md` must exist.
- `<repo-root>/skills/god/templates/guide-template.md` must exist (this is the starting point for the new guide).
- `<repo-root>/skills/god/guides/` must exist (create it if not — it's just a folder).

If `skills/god/` is missing entirely, the host repo doesn't have the God routing skill installed. Stop and ask the user how to proceed — registering against a missing God is meaningless.

### Step 2 — Read God's roster and check for collisions

Open `<repo-root>/skills/god/SKILL.md` and read it end to end. Locate the **Roster** section — it's a markdown table with columns roughly matching `Angel | Domain | Trigger keywords | Guide`.
...
```
<repo-root>/skills/god/guides/<angel-name>.md
```
...
**Path notation caveat.** God's `templates/guide-template.md` uses the current GG Coder layout. If older source material mentions Cursor or `ai-tools` paths, normalize while filling in:

- Guardian file: `agents/<angel-name>.md`
- Weapon folder: `skills/<weapon-name>/`
- Command Brief: `command-briefs/<angel-name>-command-brief.md`

Relative links in the guide (it lives at `skills/god/guides/<angel>.md`) resolve to root-level files via `../../../agents/<angel>.md`, `../../<weapon>/`, and `../../../command-briefs/<angel>-command-brief.md`.
...
Open `<repo-root>/skills/god/SKILL.md`. Add one row to the Roster table for the new Angel. Format example:
...
1. Reopen `skills/god/SKILL.md` and confirm the new roster row is present and well-formed.
2. Reopen `skills/god/guides/<angel-name>.md` and confirm every section is filled.
...
>
> - **Roster row:** added to `skills/god/SKILL.md`
> - **Guide:** authored at `skills/god/guides/<angel-name>.md`
>
...
2. That guide exists at `skills/god/guides/<angel-name>.md` with every section filled.
